async function fetchRedirect() {
  ...
  const num = Math.floor(Math.random() * 99999) + 1; // 1 to 99999
  return num.toString().padStart(5, '0'); // ensure 5 digits with leading zeros
}
...
async function createUser() {
  ...
  try {
    ...
    const body = {
      emails: ["mrx_poc_email"+num+"@gmail.com"], // ⚠️ replace with valid business email
    ...
    alert("the email "+"mrx_poc_email"+num+"@gmail.com"+" got added as the admin");
  } catch (err) {
    alert("❌ Error: " + err.message);
  }
}
